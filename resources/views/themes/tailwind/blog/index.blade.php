...
@section('content')
    <div class="relative  mx-auto xl:px-5 max-w-7xl sm:px-6 lg:pt-10">
            ...
            <div class="flex flex-col justify-start">
                <ul class="flex self-start px-3 py-1 my-6 text-xs font-medium text-gray-600 bg-blue-100 rounded-md">
                    ...
                    @foreach ($categories as $cat)
                        <li class="@if (isset($category) && isset($category->slug) && $category->slug == $cat->slug) {{ 'text-blue-700' }} @endif"><a
                                href="{{ route('wave.blog.category', $cat->slug) }}">{{ $cat->name }}</a></li>
                        @if (!$loop->last)
                            <li class="mx-2">|</li>
                        @endif
                    @endforeach
