...
@section('content')
<section class="py-12 sm:mt-44 bg-base-200 px-5 sm:px-0 rounded-xl font-work mt-44 my-6">
    <div class="flex items-center justify-center gap-4">
        <div class="avatar">
            <div class="w-16 rounded-full">
                <img style="width: 64px; height: 64px;"
                    src="{{ Voyager::image($user->avatar) ?? 'https://placehold.it/100x100'}}"
                    alt="{{ $user->name }}" />
            </div>
        </div>
        <div>
            <h5 class="text-base-content text-base sm:text-xl font-medium">{{ $user->name }}</h5>
            <span class="text-base-content/60 text-xs sm:text-sm">{{ $user->role->display_name }}</span>

        </div>
    </div>
    <p class="text-sm sm:text-lg text-base-content/70 text-center py-6 max-w-2xl w-full mx-auto">
        {{ $user->profile('about') }}</p>
    <div class="flex items-center justify-center gap-2">
        <!-- share icon 1 -->
        <a href="/" target="_blank"
            class="bg-secondary text-secondary-content hover:text-primary-content w-8 h-8 flex justify-center items-center rounded-md hover:bg-primary transition duration-300 ease-in-out">
