        ...
        <div class="w-full lg:w-9/12 xl:w-4/5">
            <div>
                ...
                <div class="mt-1 rounded-md shadow-sm">
                    <input id="name" type="text" name="name" placeholder="Name" value="{{ Auth::user()->name }}"
                        required class="input input-bordered w-full max-w-full">
            ...
            <div class="mt-5">
                ...
                <div class="mt-1 rounded-md shadow-sm">
                    <input id="email" type="text" name="email" placeholder="Email Address"
                        value="{{ Auth::user()->email }}" required class="input input-bordered w-full max-w-full">
                </div>
            </div>
            ...
            <div class="mt-5">
                ...
                <div class="mt-1 rounded-md">
                    ...
                    <textarea class="textarea textarea-bordered textarea-lg w-full max-w-full" rows="5"
                        placeholder="About" name="about">{{ Auth::user()->profile('about') }}</textarea>
                </div>
            </div>

            <div class="flex justify-end w-full">
                <button class="btn btn-outline btn-info px-4 py-2 mt-5" dusk="update-profile-button">Save</button>
            </div>
        </div>
    </div>
...
<div id="uploadModal" x-data x-init="$watch('$store.uploadModal.open', value => {
    ...
    <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <div x-show="$store.uploadModal.open" @click="$store.uploadModal.open = false;"
        ...
        <div x-show="$store.uploadModal.open" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="mt-2">
                        <div id="upload-crop-container" class="relative flex items-center justify-center h-56 mt-5">
                            <div id="uploadLoading" class="flex items-center justify-center w-full h-full">
                                <svg class="w-5 h-5 mr-3 -ml-1 text-gray-400 animate-spin"
            ...
            <div class="mt-5 sm:mt-6">
                <span class="flex w-full rounded-md shadow-sm">
                    <button @click="$store.uploadModal.open = false;"
                        class="inline-flex justify-center w-full px-4 py-2 mr-2 text-base font-medium leading-6 text-gray-700 transition duration-150 ease-in-out bg-white border border-transparent border-gray-300 rounded-md shadow-sm hover:text-gray-500 active:text-gray-800 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue sm:text-sm sm:leading-5"
                        type="button">Cancel</button>
                    <button @click="$store.uploadModal.open = false; window.applyImageCrop()"
                        class="inline-flex justify-center w-full px-4 py-2 ml-2 text-base font-medium leading-6 text-white transition duration-150 ease-in-out border border-transparent rounded-md shadow-sm bg-wave-600 hover:bg-wave-500 focus:outline-none focus:border-wave-700 focus:shadow-outline-wave sm:text-sm sm:leading-5"
