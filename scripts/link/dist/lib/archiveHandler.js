...
export default async function archiveHandler(link) {
    const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error(`Browser has been open for more than ${BROWSER_TIMEOUT} minutes.`)), BROWSER_TIMEOUT * 60000);
    });
        ...
        await Promise.race([
            (async () => {
                ...
                if (linkType === "image" && !link.image?.startsWith("archive")) {
                    ...
                    // Preview
                    if (!link.preview?.startsWith("archives") &&
                    ...
                    // Monolith
                    if (!link.monolith?.startsWith("archive") &&
