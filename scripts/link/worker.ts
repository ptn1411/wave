...
async function processBatch() {
    ...
    const archiveLink = async (link: links) => {
        try {
            console.log(
                "\x1b[34m%s\x1b[0m",
                `Processing link ${link.url} for user ${link.id}`
            );
            ...
            console.log(
                "\x1b[34m%s\x1b[0m",
                `Succeeded processing link ${link.url} for user ${link.id}.`
            );
        } catch (error) {
            console.error(
                "\x1b[34m%s\x1b[0m",
                `Error processing link ${link.url} for user ${link.id}:`,
                error
            );
        }
    };
